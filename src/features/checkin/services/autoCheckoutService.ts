...
export class AutoCheckoutService extends BaseService {
  ...
  async processAutoCheckouts(): Promise<number> {
    try {
      ...
      console.log(
        `[AutoCheckout] Found ${activeCheckins.length} active checkins`
      )

      let processedCount = 0

      for (const checkin of activeCheckins) {
        const shouldCheckout = await this.shouldAutoCheckout(
          checkin,
          checkin.character
        )
  ...
  private shouldAutoCheckout(checkin: any, character: any): boolean {
    ...
    if (hoursWorked >= AutoCheckoutService.AUTO_CHECKOUT_CONFIG.maxWorkHours) {
      console.log(
        `[AutoCheckout] Character ${character.id} worked ${hoursWorked.toFixed(2)} hours (max: ${AutoCheckoutService.AUTO_CHECKOUT_CONFIG.maxWorkHours})`
      )
      return true
    }
    ...
    if (character.workStartTime && character.workEndTime) {
      ...
      if (now >= checkoutTime) {
        console.log(
          `[AutoCheckout] Character ${character.id} passed auto checkout time`
        )
        return true
      }
    } else {
      ...
      if (now >= midnight) {
        console.log(
          `[AutoCheckout] Character ${character.id} no work schedule, checkout at midnight`
        )
        return true
      }
    }
  ...
  private calculateAutoCheckoutTime(checkin: any, character: any): Date {
    ...
    if (character.workEndTime) {
      ...
      if (
        ...
        character.workEndTime < character.workStartTime
      ) {
      ...
      checkoutTime.setHours(
        checkoutTime.getHours() +
          AutoCheckoutService.AUTO_CHECKOUT_CONFIG.bufferHours
      )
  ...
  private async performAutoCheckout(checkin: any): Promise<void> {
    ...
    const totalHours =
      (checkoutTime.getTime() - new Date(checkin.checkinAt).getTime()) /
      (1000 * 60 * 60)
    ...
    console.log(
      `[AutoCheckout] Auto checked out character ${checkin.character.id} at ${checkoutTime.toISOString()}`
    )
  ...
  private calculateCheckoutTime(checkin: any, character: any): Date {
    ...
    // ถ้ามีเวลาเลิกงาน
    if (character.workEndTime) {
      const autoCheckoutTime = this.calculateAutoCheckoutTime(
        checkin,
        character
      )
  ...
  private generateAutoCheckoutNote(checkin: any, character: any): string {
    const hoursWorked =
      (new Date().getTime() - new Date(checkin.checkinAt).getTime()) /
      (1000 * 60 * 60)
  ...
  async checkAndNotifyPendingAutoCheckouts(): Promise<void> {
    ...
    for (const checkin of pendingCheckins) {
      ...
      if (autoCheckoutTime >= now && autoCheckoutTime <= thirtyMinutesFromNow) {
        console.log(
          `[AutoCheckout] Character ${checkin.character.id} will be auto checked out in 30 minutes`
        )
