      ...
      <div className="flex items-center space-x-2">
        ...
        <span className="text-sm font-medium text-foreground">
          {displayUsers.length === 1 ? (
            <span>
              {displayUsers[0].user.character?.name ||
                displayUsers[0].user.name}
            </span>
            ...
            <span>
              {displayUsers[0].user.character?.name ||
                displayUsers[0].user.name}{' '}
              และ{' '}
              {displayUsers[1].user.character?.name ||
                displayUsers[1].user.name}
            </span>
            ...
            <span>
              ...
              {displayUsers[1].user.character?.name ||
                displayUsers[1].user.name}{' '}
              และ{' '}
              {displayUsers[2].user.character?.name ||
                displayUsers[2].user.name}
            </span>
          ) : (
            `${totalLikes} คน`
          )}{' '}
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {likeUsers.map((likeUser) => (
              <div
                key={likeUser.id}
                className="flex items-center space-x-3 p-3 rounded-xl hover:bg-muted/50">
