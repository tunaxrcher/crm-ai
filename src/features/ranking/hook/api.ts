...
import { GetRankingsParams, GetRankingsResponse, CharacterClass, RankingPeriod } from '../types';
...
export function useRankings() {
  ...
  const getOrderedRankings = useCallback(() => {
    ...
    return rankings.filter(user =>
      ...
      !(currentUser && currentUser.id !== topUser?.id && user.id === currentUser.id)
    );
  }, [data]);
...
export function useClassConfig() {
  ...
  const fetchClassConfig = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch class config'));
    } finally {
      setIsLoading(false);
    }
  }, []);
