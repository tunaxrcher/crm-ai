...
import { getTeams, getTeamDetails, getTeamQuests, joinTeam, createTeam } from '../service/client';
...
export function useTeamDetails(teamId: string) {
  ...
  const fetchTeamDetails = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error(`Failed to fetch team details for ${teamId}`));
    } finally {
      setLoading(false);
    }
  }, [teamId]);
...
export function useTeamQuests() {
  ...
  const fetchQuests = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch team quests'));
    } finally {
      setLoading(false);
    }
  }, []);
...
export function useJoinTeam() {
  ...
  const requestJoin = useCallback(async (teamId: string, message: string) => {
    try {
      ...
      const error = err instanceof Error ? err : new Error(`Failed to join team ${teamId}`);
      setError(error);
      throw error;
    } finally {
      setLoading(false);
...
export function useCreateTeam() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [result, setResult] = useState<{ success: boolean; teamId?: string; message: string } | null>(null);

  const create = useCallback(async (teamData: {
    name: string;
    description: string;
    isPrivate: boolean;
    tags: string[];
