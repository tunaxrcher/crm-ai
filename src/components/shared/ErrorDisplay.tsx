...
export default function ErrorDisplay({
  ...
  const getIcon = () => {
    switch (severity) {
      ...
      case "warning":
        return <AlertTriangle className={`${getSizeClass("icon")} text-amber-400`} />;
      case "error":
        return <AlertCircle className={`${getSizeClass("icon")} text-red-400`} />;
      case "critical":
  ...
  const getSizeClass = (element: "container" | "icon" | "title" | "message" | "button") => {
    switch (element) {
      case "container":
        return size === "small" ? "p-2" : size === "medium" ? "p-4" : "p-6";
      case "icon":
        return size === "small" ? "h-5 w-5" : size === "medium" ? "h-8 w-8" : "h-12 w-12";
      case "title":
        return size === "small" ? "text-sm font-medium" : size === "medium" ? "text-base font-medium" : "text-lg font-medium";
      case "message":
        return size === "small" ? "text-xs" : size === "medium" ? "text-sm" : "text-base";
      case "button":
        return size === "small" ? "h-8 text-xs" : size === "medium" ? "h-9" : "h-10";
      default:
        return "";
    }
  };
  ...
  const content = (
    ...
    >
      {size === "small" ? (
        <div className="flex items-start">
          ...
          <div className="flex-1">
            ...
            {message && <p className={`${getSizeClass("message")} text-muted-foreground mt-1`}>{message}</p>}
          </div>
        </div>
      ) : (
        <>
          ...
          {message && <p className={`${getSizeClass("message")} text-muted-foreground mb-4 max-w-md`}>{message}</p>}
        </>
      )}
      ...
      {showRetry && onRetry && size !== "small" && (
        <Button onClick={onRetry} className={`flex items-center ${getSizeClass("button")}`}>
