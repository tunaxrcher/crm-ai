...
//     try {
//       // @ts-ignore - ใช้ $queryRawUnsafe เพื่อลบข้อมูลทั้งหมดจากตาราง
...
//   // รีเซ็ต sequence ของ ID
//   try {
//     for (const table of tablesToClean) {
//       await prisma.$executeRawUnsafe(`ALTER SEQUENCE "${table}_id_seq" RESTART WITH 1;`);
//     }
//     console.log('🔄 รีเซ็ต sequence ของ ID เรียบร้อย');
//   } catch (error) {
//     console.log(`⚠️ ไม่สามารถรีเซ็ต sequence ได้: ${error}`);
//   }
// ========== ฟังก์ชันสำหรับลบข้อมูลเก่า (แก้ไขสำหรับ MySQL แบบปลอดภัย) ==========
async function cleanupDB() {
