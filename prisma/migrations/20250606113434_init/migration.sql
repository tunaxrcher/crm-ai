...
CREATE TABLE `QuestSubmission` (
    ...
    `characterId` INTEGER NOT NULL,
    `questId` INTEGER NOT NULL,
    `assignedQuestId` INTEGER NOT NULL,

    UNIQUE INDEX `QuestSubmission_assignedQuestId_key`(`assignedQuestId`),
...
-- AddForeignKey
ALTER TABLE `QuestSubmission` ADD CONSTRAINT `QuestSubmission_assignedQuestId_fkey` FOREIGN KEY (`assignedQuestId`) REFERENCES `AssignedQuest`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
